	...
	def OneStepRMC(self,rdf):
		while True:
			...
			for j in range(len(self.balls)):
				if i != j:
					d = ball.sqdistance(self.balls[j].pos,self.cell)
					if d < 36.0:
						d = int(d*10)
						L0 *= rdf.bin[d]/rdf.count/10.0
						target.append(self.balls[j])
						if rdf.bin[d] == 0.0:
							N0 += 1
			dim = len(self.cell)
			v = [0.0] * dim
			...
			N1 = 0
			for b in target:
				d = b.sqdistance(newpos,self.cell)
				d = int(d*10)
				L1 *= rdf.bin[d]/rdf.count/10.0
				if rdf.bin[d] == 0.0:
					N1 += 1
			if L1 > L0 or (L1 == L0 and N1 <= N0): #random.random() < 0.001):
				ball.pos = newpos
				print L0,L1,N0,N1
				self.step += 1
				return (N1==0)
...
def setup():
	global system,rdf,rdf2
	width = 10.0
	height = 10.0
	zoom = 30.0
	size(width*zoom, (height+5)*zoom)
	gc = GC(zoom=zoom)
	system=System(step=0, nballs=82, cell=[width,height],
				   gc=gc,kT=1.0,MC=True)
	rdf = RDF(cell=[width,height],gc=gc)
	rdf2 = RDF(cell=[width,height],gc=gc)
...
def draw():
	...
	#時間0.1だけアニメーションを進める。
	#system.OneStep(0.1)
	if system.step < 20000: 
		for i in range(1000):
			system.OneStepMC()
	elif system.step < 40000:
		for i in range(1000):
			system.OneStepMC()
		rdf.accum(system.balls)
		rdf.draw()
	else:
		if system.step == 40000:
			for ball in system.balls:
				ball.pos = [random.random()*system.cell[0],
				            random.random()*system.cell[1]]
			system.delta=1.0
		for i in range(10):
			a = system.OneStepRMC(rdf)
			if a:
				system.delta = 0.1
			else:
				system.delta = 1.0
